...
for Lambda_w in range(10):
    ...
    if total_cost_static >0 :
        ...
        plt.title(f"A.Resource Utilization λ={Lambda_List[0]}")
        plt.xlabel('Hour')
        plt.ylabel('δ ')
        plt.legend()
        ...
        plt.subplot(2, 2, 2)
        plt.plot(provider_utility_list,marker='x', label='provider utility')
        plt.plot(client_utility_list,marker='s', label='Client utility')
        plt.title('B.Utilities')
        plt.xlabel('Hour')
        ...
        plt.plot(cost_dynamic_array,marker='s', label='Dynamic Pricing')
        plt.title('C.cost ')
        plt.xlabel('Hour')
        plt.ylabel('Cost  ($)')
        plt.legend()
